...
}(), window.Port = window.Port || {}, function() {
    var $port = $("#port"), initAnalytic = function() {
        var link = $(this);
...
angular.module("proCompra.controllers", []).controller("QuestionaryCtrl", [ "$scope", "$window", "Questionary", "$filter", "$rootScope", "Auth", function($scope, $window, Questionary, $filter) {
    ...
    $scope.questionary = null, $scope.scrollTop = function() {
        ...
        if (n) {
            $scope.wizard.id = n.id, $scope.wizard.uri = n.uri, $scope.is_price_calculator_active = n.is_price_calculator_active || !1, 
            $scope.is_accept_contact_active = n.is_accept_contact_active || !1, questionaryAcceptContactLength = n.questions.length;
            for (var i = n.questions.length - 1; i >= 0; i--) n.questions[i].options.length > 4 && ($scope.bigQuestion = !0), 
            n.questions[i].is_base && ($scope.totalPrice.defaultMin = n.questions[i].defaultValues.min_value, 
            $scope.totalPrice.defaultMax = n.questions[i].defaultValues.max_value);
        }
    }), $scope.$watch("wizard", function(n) {
        var totalMin = 0, totalMax = 0;
        if ($scope.is_accept_contact_active || n.step != questionaryAcceptContactLength + 1 || (++n.step, 
        n.accept_contact = "1"), n.id && n.answers && n.answers[n.answerCurrent]) {
            if (n.answers) if (1 == Object.keys(n.answers).length) for (x in n.answers) n.answers[x].is_base && ($scope.totalPrice.min = n.answers[x].minValue, 
            $scope.totalPrice.max = n.answers[x].maxValue, $scope.totalPrice.is_only_base = !0); else $scope.totalPrice.is_only_base = !1;
            if ($scope.totalPrice.min && $scope.totalPrice.max) {
                for (price in n.answers) $scope.totalPrice.is_only_base || n.answers[price].is_base || (totalMin += n.answers[price].minValue, 
                totalMax += n.answers[price].maxValue);
                $scope.totalPrice.is_only_base || ($scope.totalPrice.min = totalMin, $scope.totalPrice.max = totalMax), 
                $scope.totalPrice.status = !0;
            } else $scope.totalPrice = {
                status: !0,
                min: n.answers[n.answerCurrent].minValue,
                max: n.answers[n.answerCurrent].maxValue
            };
        }
    }, !0), $scope.isEmpty = function(obj) {
        var status = !1;
        if ($scope.questionary && $scope.questionary.questions.length) for (var i = 0; i < $scope.questionary.questions.length; i++) if ($scope.questionary.questions[i].options) for (var z = 0; z < $scope.questionary.questions[i].options.length; z++) obj == $scope.questionary.questions[i].options[z].id && (status = !0);
        return status;
    }, $scope.loading = null, $scope.accept_contact = null, $scope.totalPrice = {
        status: !1,
        min: !1,
        max: !1
    }, document.getElementById("fieldPhone").addEventListener("keyup", function() {
        this.value && "0" === this.value[1] && (this.value = this.value.substr(2, this.value.length));
    });
    var normalization = function(s) {
        var res = _.deburr(s);
        return res = _.trim(res).replace(/\s/g, "_"), res.toLowerCase();
    };
    $scope.prev = function() {
        $scope.wizard.step--, ga("send", "event", "Questionnaire", "Step_" + $scope.wizard.step, "Btn_Click_Prev");
    }, $scope.next = function() {
        $scope.wizard.step++, ga("send", "event", "Questionnaire", "Step_" + $scope.wizard.step, "Btn_Click_Next"), 
        $scope.wizard.step == $scope.questionary.questions.length + 2 && (ga("send", "event", "Questionnaire", "Question_AcceptTerms", "Btn_Click_" + $scope.wizard.accept_contact), 
        dataLayer.push({
            event: "inputPage"
        }));
    }, $scope.setAnswer = function(q, o) {
        $scope.wizard.answers = $scope.wizard.answers || {}, $scope.wizard.answers[q.id] = {
            id: o.id,
            minValue: o.minValue,
            maxValue: o.maxValue,
            is_base: q.is_base,
            defaultValues: q.defaultValues
        }, $scope.wizard.answerCurrent = q.id, ga("send", "event", "Questionnaire", "Question_" + normalization(q.title), "Btn_Click_Answer_" + normalization(o.text)), 
        $scope.next();
    }, $scope.submit = function() {
        var isOk = !0;
        $scope.submitted = !0, $scope.error = {
            name: null,
            email: null,
            phone: null
        }, null == $scope.wizard.name && ($scope.error.name = "Campo obrigatório", isOk = !1), 
        null == $scope.wizard.email ? ($scope.error.email = "Campo obrigatório", isOk = !1) : $filter("isEmail")($scope.wizard.email) || ($scope.error.email = "O email precisa ser válido", 
        isOk = !1), null == $scope.wizard.phone && ($scope.error.phone = "Campo obrigatório", 
        isOk = !1), $scope.wizard.postal && null != $scope.wizard.postal || ($scope.error.postal = "Campo obrigatório", 
        isOk = !1), isOk && ($scope.loading = !0, $scope.wizard.answers ? ($scope.wizard.min_value = $scope.totalPrice.min, 
        $scope.wizard.max_value = $scope.totalPrice.max) : ($scope.wizard.min_value = $scope.totalPrice.defaultMin, 
        $scope.wizard.max_value = $scope.totalPrice.defaultMax), Questionary.post($scope.wizard).then(function(r) {
            $scope.upload(r.data.incrementId, r, function(obj, r) {
                var qtdAnswers = $scope.wizard.answers || 0;
                qtdAnswers && "object" == typeof qtdAnswers && (qtdAnswers = Object.keys($scope.wizard.answers).length), 
                ...
                window.optimizely = window.optimizely || [], window.optimizely.push([ "trackEvent", "purchase" ]), 
                $scope.questionary.incrementId = r.data.incrementId, $scope.loading = null, $scope.next(), 
...
} ]), angular.module("proCompra.services", []).factory("SerializeWizard", [ "$filter", function($filter) {
    return {
        get: function(data) {
            var buffer = [];
            for (var step in data.answers) buffer.push(encodeURIComponent("wizard[step" + step + "][" + step + "]") + "=" + data.answers[step].id);
    ...
    }, i = 0; i < app.environment.length; i++) {
    ...
    return questionary.get = function(category) {
        ...
        var filesList = [];
        if (!brid) return !1;
        for (var i = 0; i < files.length; i++) files[i].name && filesList.push(files[i].name);
        return $http.post(Auth.env() + "/save-request-files-list", {
            incrementId: brid,
            files: filesList
        });
